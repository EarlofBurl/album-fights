...
<div style="width: 100%; max-width: 1000px; margin: 0 auto; background: var(--card-bg); padding: 30px; border-radius: 12px; border: 1px solid var(--border);">
    ...
    <p style="color: var(--text-muted); margin-bottom: 30px;">Albums you need to give another listen before they face the tough duel land here.</p>

    <?php if (empty($queue)): ?>
        <p style="text-align: center; color: #666; font-style: italic; padding: 40px 0;">The queue is currently empty.</p>
    <?php else: ?>
            ...
            <tbody>
                <?php foreach ($queue as $index => $album): ?>
                    <tr style="border-bottom: 1px solid var(--border);">
                        <td style="padding: 15px 10px; font-weight: bold; color: var(--accent);"><?= htmlspecialchars((string)$album['Artist']) ?></td>
                        <td style="padding: 15px 10px;"><?= htmlspecialchars((string)$album['Album']) ?></td>
                        <td style="padding: 15px 10px; color: var(--text-muted); font-size: 0.9rem;">
                            Elo: <?= round((float)$album['Elo']) ?> | Duels: <?= (int)$album['Duels'] ?><br>
                            W/L: <?= (int)($album['Wins'] ?? 0) ?>/<?= (int)($album['Losses'] ?? 0) ?> (<?= htmlspecialchars(CsvHelper::winLossRatio((int)($album['Wins'] ?? 0), (int)($album['Losses'] ?? 0))) ?>)
                        </td>
                        <td style="padding: 15px 10px; text-align: right;">
                            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                                <form method="POST" style="margin: 0;">
                                    ...
                                    <button type="submit" class="btn-small" style="background-color: #4CAF50; color: white; width: 140px;" title="Back to duels">🔙 Restore</button>
                                </form>
                                <form method="POST" style="margin: 0;" onsubmit="return confirm('Do you really want to delete this album from the database forever?');">
                                    ...
                                    <input type="hidden" name="targetIdx" value="<?= $index ?>">
                                    <button type="submit" class="btn-small btn-delete" style="width: 120px;" title="Delete completely">🗑️ Delete</button>
                                </form>
                            </div>
                        </td>
                    </tr>
